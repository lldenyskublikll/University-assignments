...
class ImageProcessingApp:
    def __init__(self, root):
        ...
        # Створення кноки "Використати фільтр "Emboss"" 
        self.filter_button = Button(root, text = "Apply 'EMBOSS' filter", command = self.apply_emboss_filter)
        self.filter_button.grid(row = 5, column = 0)
...
# Метод для зміни розміру зображення
    def resize_image(self):
        if self.original_image:
            ...
            self.original_image = self.original_image.resize((width, height)) # Зміна розмірів зображення
            self.processed_image = self.original_image
            self.label.config(text = f"Status: Image size changed on {width} x {height}")
            self.show_preview() # Виклик методу для відображення обробленого зображення
          
# Метод для обробки зображення фільтром "EMBOSS"
    def apply_emboss_filter(self):
        if self.original_image:
            self.original_image = self.original_image.filter(ImageFilter.EMBOSS) # Обробка зображення обраним фільтром
            self.processed_image = self.original_image
            self.label.config(text = "Status: 'EMBOSS' filter applied")
            self.show_preview() # Виклик методу для відображення обробленого зображення
